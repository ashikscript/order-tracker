...
function toggleTheme() {
  ...
  applyTheme(next);
  try { localStorage.setItem("orderTrackerTheme", next); } catch (e) {}
}

function initTheme() {
  let saved = "dark";
  try { saved = localStorage.getItem("orderTrackerTheme") || "dark"; } catch (e) {}
  applyTheme(saved);
}
...
function setAdmin(on) {
  ...
  const guestNote = document.getElementById("guestSummaryNote");
  ...
  if (on) {
    ...
    lockEls.forEach(el => { if (el) el.disabled = false; });
...
function exportExcel() {
  if (!isAdmin) { alert("🔒 Please login as admin to export."); return; }
